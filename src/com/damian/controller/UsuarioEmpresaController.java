...
public class UsuarioEmpresaController {
	...
	public ModelAndView getUsers(ModelAndView modelAndView, @PathVariable("idUsr") int idUsr,
		...
		modelAndView.addObject("empresas", empresaService.findAll(null, 0, 10000, request));
	...
	public ModelAndView getCompanies(ModelAndView modelAndView, @PathVariable("idEmp") int idEmp,
		...
		modelAndView.addObject("usuarios", usuarioService.findAll("datosPersonales.nombre", "ASC", 0, 10000, request));
		modelAndView.setViewName("empresaUsuario");
		return modelAndView;
	}

	@RequestMapping("/usuarioEmpresa/usuario/save/{idUsr}")
	public String saveUserRelation(ModelAndView modelAndView, @PathVariable("idUsr") int idUsr,
			HttpServletRequest request, RedirectAttributes ra, @ModelAttribute("auxString") AuxString auxString) {
		try {
	...
	public String saveCompanyRelation(ModelAndView modelAndView, @PathVariable("idEmp") int idEmp,
			HttpServletRequest request, RedirectAttributes ra, @ModelAttribute("auxString") AuxString auxString) {
		try {
			...
			ra.addFlashAttribute("existia", usuario.getDatosPersonales().getNombre() + " "
					+ usuario.getDatosPersonales().getApellido1() + " " + usuario.getDatosPersonales().getApellido2());
			ra.addFlashAttribute("error", true);
		}
