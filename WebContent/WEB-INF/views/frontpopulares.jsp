...
<html>
<head>
	<title><fmt:message key="label.Promotion2" /></title>
				...
				<div class="bloqueProducto">
					<c:if test="${(not empty popular.producto.nombreFotoPrincipal)}">
						<div class="nombreProducto">
							<c:out value="${popular.producto[nameColSelect]}"/>
						</div>
						<c:if test="${popular.producto.favorito}">
						<img src='<c:url value="/resources/imgs/favoritos/${prefUsr.botonFavorito}-yes.png"/>' class="tamanio_imagen cursor-pointer" onclick='marcarFavorito(${popular.producto.idPro},false)'>
						</c:if>
						<c:if test="${(not popular.producto.favorito && not empty prefUsr.botonFavorito)}">
						<img src='<c:url value="/resources/imgs/favoritos/${prefUsr.botonFavorito}-no.png"/>' class="tamanio_imagen cursor-pointer" onclick='marcarFavorito(${popular.producto.idPro},true)'>
						</c:if>
						<div class="imagenProducto">
							<a title="${popular.producto[nameColSelect]}" onclick='location.href="<c:url value='/detalle/producto/${popular.producto.idPro}' />"' class="cursor-pointer">
								<c:if test="${popular.booleanOferta}">
									<h4 class="precioSinOferta"><fmt:formatNumber type="currency" value="${popular.precioSinOferta}" /></h4>
									<h2 class="precioConOferta"><fmt:formatNumber type="currency" value="${popular.precioConOferta}" /></h2>
								</c:if>
								<c:if test="${not popular.booleanOferta}">
									<h2 class="precioConOferta"><fmt:formatNumber type="currency" value="${popular.producto.precioVenta}" /></h2>
								</c:if>
